"""
Step 6-7: Configure Training and Train Model (Intel XPU Support)
Fine-tune the model using SFTTrainer with Intel Extension for PyTorch
Supports Intel Arc A/B-Series, Core Ultra Graphics, and Data Center GPU Max Series
"""
...
def configure_training(device_type, quantization_bits=8):
    ...
    if device_type == "cuda":
        ...
        # Intel GPU (XPU)用設定：量子化なし
        model_kwargs['device_map'] = "xpu"
        model_kwargs['torch_dtype'] = torch.bfloat16
        optim = "adamw_torch"
    else:
        # CPU用設定：量子化なし
        model_kwargs['device_map'] = "cpu"
        model_kwargs['torch_dtype'] = torch.float32 # torch.float32 | torch.bfloat16
        optim = "adamw_torch"
